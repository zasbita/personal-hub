...
use App\Services\{DisplayTime, FootballService, MotoGPService, NameMatcher, SportPrefsService, SupabaseService, TelegramService, VolleyballService};
...
class MatchNotifier extends Command
{
    ...
    private function notifyMotoGP(SupabaseService $s, TelegramService $tg, array $prefs): void
    {
        try {
            ...
            $all = [];
            foreach (['motogp', 'moto2', 'moto3', 'baggers'] as $c) { foreach ($moto->getCurrentSeasonRaces($c) as $r) { $all[] = array_merge($r, ['classification' => $c]); } }

...
use App\Services\{ExpenseSummary, SheetsService, SupabaseService, TelegramService, VehicleService};
...
class WeeklyDigest extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (!$owner) { $this->error('OWNER_ID not configured'); return 1; }

        $days = max(1, (int) $this->option('days'));
        $tg = new TelegramService();
        $parts = array_filter([$this->expenses($days), $this->service($owner)]);
        if (empty($parts)) { $this->info('Nothing to report'); return 0; }
    ...
    private function expenses(int $days): string
    {
        try {
            ...
            if (empty($r['items'])) return "📅 *Laporan Mingguan*\nBelum ada pengeluaran {$days} hari terakhir.";
    ...
    private function service(int $owner): string
    {
        try {
            ...
            if (!$v) return '';
            $left = $v['remainingKm'];
            if ($left > self::SERVICE_WARN_KM) return ''; // nothing useful to say yet
            $head = $left <= 0
